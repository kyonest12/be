...
export class FriendController {
    ...
    @HttpCode(200)
    async getRequestedFriends(@AuthUser() user: User, body: GetListDto) {
        return this.friendService.getRequestedFriends(user, body);
    }

    @Post('/set_request_friend')
    @HttpCode(200)
    async setRequestFriend(@AuthUser() user: User, body: SetRequestFriendDto) {
        return this.friendService.setRequestFriend(user, body);
    }

    @Post('/set_accept_friend')
    @HttpCode(200)
    async setAcceptFriend(@AuthUser() user: User, body: SetAcceptFriend) {
        return this.friendService.setAcceptFriend(user, body);
    }

    @Post('/get_user_friends')
    @HttpCode(200)
    async getUserFriends(@AuthUser() user: User, body: GetListFriendsDto) {
